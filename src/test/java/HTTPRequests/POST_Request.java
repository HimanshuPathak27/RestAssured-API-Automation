...
public class POST_Request {
	...
	public void create_a_user() {
		...
		jsonData.put("job", "API Automation QA");
		...
		Assert.assertEquals(statusCode, 201, "Failed | Satus Code is not correct");
		System.out.println();

		//Validate Status Line
		String statusLine = response.getStatusLine();
		System.out.println("Status Line: " + statusLine);
		Assert.assertEquals(statusLine, "HTTP/1.1 201 Created", "Failed | Satus Line is not correct");
		System.out.println();
		...
		Assert.assertEquals(job, "API Automation QA", "Failed | Job Mismatched");
